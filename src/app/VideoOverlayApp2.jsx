...
const VideoTextOverlayApp = () => {
  ...
  const startActualRecording = () => {
    ...
    webcamRecorderRef.current.onstop = async () => {
      ...
      try {
        // Create a video element to load the recorded video
        ...
        // Create canvas for rotation (swap width/height for 90deg rotation)
        const rotateCanvas = document.createElement("canvas");
        rotateCanvas.width = sourceVideo.videoHeight;
        rotateCanvas.height = sourceVideo.videoWidth;
        const ctx = rotateCanvas.getContext("2d");

        // Create audio context to extract audio from original video
        const audioContext = new AudioContext();
        const sourceVideo2 = document.createElement("video");
        sourceVideo2.src = URL.createObjectURL(blob);
        await sourceVideo2.play();

        const audioSource = audioContext.createMediaElementSource(sourceVideo2);
        const audioDestination = audioContext.createMediaStreamDestination();
        audioSource.connect(audioDestination);

        // Setup MediaRecorder for rotated video with audio
        const rotatedStream = rotateCanvas.captureStream(30);
        audioDestination.stream.getAudioTracks().forEach((track) => {
          rotatedStream.addTrack(track);
        });

        const rotatedRecorder = new MediaRecorder(rotatedStream, {
          mimeType: "video/webm",
        });
        const rotatedChunks = [];

        rotatedRecorder.ondataavailable = (e) => rotatedChunks.push(e.data);
        rotatedRecorder.onstop = () => {
          const rotatedBlob = new Blob(rotatedChunks, { type: "video/webm" });
          const url = URL.createObjectURL(rotatedBlob);

          // Load the rotated video into the editor
          setVideoUrl(url);
        ...
        rotatedRecorder.start();
        sourceVideo.play();

        // Draw rotated frames
        const drawFrame = () => {
          if (sourceVideo.ended || sourceVideo.paused) {
            rotatedRecorder.stop();
            return;
          }
  ...
  const startRecording = async () => {
    ...
    canvasRef.current.width = 405;   // Force 720p width
    canvasRef.current.height = 720;  // Force 720p height
        ...
        <div className="flex flex-col lg:flex-row gap-6 h-full w-full">
          ...
          <div className="flex-1 flex flex-col justify-center items-center ">
            <div className="p-6 w-full flex flex-col justify-center items-center">
              <div className="relative bg-red-400 rounded-lg overflow-hidden mb-4 w-full hfull">
                ...
                {isWebcamActive && !videoUrl && (
                  <div className="relative h-svh flex justify-center items-center">
                    <video
                      ref={webcamVideoRef}
                      autoPlay
                      playsInline
                      muted
                      className="absolute inset-0 w-full h-full object-contain transform rotate-90 scale-[1.78]"
                    />
